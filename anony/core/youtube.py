...
# ── Downloader 2: New YouTube API ────────────────────────────────────────────────────
async def _xbit_download(link: str, media_type: str) -> str | None:
    """
    Download via new YouTube API.
    GET {YTPROXY_URL}/audio? id=<video_id> → stream download.
    Returns local file path on success, None on failure.
    """
    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            endpoint = "/video" if media_type == "video" else "/audio"
            async with session.get(
                f"{YTPROXY_URL}{endpoint}",
                params={"id": video_id, "platform": "youtube"},
                timeout=aiohttp.ClientTimeout(total=20),
            ) as resp:
                if resp.status != 200:
                    logger.warning("New API info failed: status %s", resp.status)
                    return None
                data = await resp.json()

            media_data = data.get("video") if media_type == "video" else data.get("audio")
            if not media_data:
                logger.warning("New API: no %s data in response", media_type)
                return None

            media_url = media_data.get("url")
            if not media_url:
                logger.warning("New API: no %s url in response", media_type)
                return None
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
            logger.info("New API ✓ %s → %s", video_id, file_path)
            return file_path

        return None

    except Exception as exc:
        logger.warning("New API download failed for %s: %s", video_id, exc)
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
        except OSError:
